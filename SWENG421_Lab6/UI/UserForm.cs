...
public partial class UserForm : Form {
    ...
    public UserForm(string userName) {
        ...
        RefreshGraphList();
        _manager.GraphsChanged += (s, e) => RefreshGraphList();
        _manager.GraphUpdated += (s, updatedGraphId) => {
            if (_displayedGraph?.GraphId == updatedGraphId)
    ...
    private void btnCopy_Click(object sender, EventArgs e) {
        var g = SelectedGraph();
        if (g == null) {
            MessageBox.Show("Select a graph to copy.", "No selection");
            return;
        }
    ...
    private void btnDisplay_Click(object sender, EventArgs e) {
        var g = SelectedGraph();
        if (g == null) {
            MessageBox.Show("Select a graph to display.", "No selection");
            return;
        }
    ...
    private void btnAddVertex_Click(object sender, EventArgs e) {
        var g = SelectedGraph();
        if (g == null) {
            MessageBox.Show("Select a graph first.", "No selection");
            return;
        }
    ...
    private void btnUpdateVertex_Click(object sender, EventArgs e) {
        ...
        using var dlg = new UpdateVertexDialog();
        if (dlg.ShowDialog() != DialogResult.OK) return;

        try {
            _manager.UpdateVertex(g.GraphId, dlg.VertexId, dlg.NewX, dlg.NewY);
            SetStatus($"Updated V{dlg.VertexId} in Graph {g.GraphId}");
            pnlCanvas.Invalidate();
        }
        catch (Exception ex) {
            MessageBox.Show(ex.Message, "Error");
    ...
    private void btnUpdateEdge_Click(object sender, EventArgs e) {
        ...
        using var dlg = new UpdateEdgeDialog();
        if (dlg.ShowDialog() != DialogResult.OK) return;

        try {
            _manager.UpdateEdge(g.GraphId, dlg.EdgeId, dlg.NewFromId, dlg.NewToId);
            SetStatus($"Updated E{dlg.EdgeId} in Graph {g.GraphId}");
            pnlCanvas.Invalidate();
        }
        catch (Exception ex) {
            MessageBox.Show(ex.Message, "Error");
